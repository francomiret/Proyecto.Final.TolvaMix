<template>
  <MySection :title="`Control de calidad - ${item.title}`">
    <MyInfo>
      El nuevo estado de la planilla será
      <strong>{{ $t(item.status.toLocale()) }}</strong>
    </MyInfo>
    <MySection title="Detalles">
      <MyTable :items="item.details">
        ...
        <template #item.actions="{ item }">
          <v-icon @click="clickApprove(item)" title="Aprobar"
          ...
          <TheFormDialog @confirm="saveRejectedDetail(item)">
            ...
            <v-form>
              <v-textarea
                ...
                outlined
                autofocus
                v-model="editDetail.justification"
                label="Justificación"
              />
            </v-form>
          </TheFormDialog>
...
export default {
  ...
  methods: {
    ...
    updateStatus() {
      if (
        this.item.details.findIndex(
          (detail) => !detail.status.equals(QualityDetailStatus.APPROVED)
        )
      ) {
